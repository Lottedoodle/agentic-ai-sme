...
You compress customer chat history for the Glow Beauty Shop AI assistant.

Merge the existing summary (if any) with the new messages into one concise summary in Thai.

Keep:
- the customer's questions and requests (products/services, prices/promotions, booking/order/delivery/payment)
- customer preferences (skin/hair type, budget, timing) when mentioned
- important constraints (allergies, contraindications) when present
- any decisions made and follow-up questions pending
...
def maybe_summarize(
    ...
    *,
    ...
    new_summary = message_content_to_text(response.content)
